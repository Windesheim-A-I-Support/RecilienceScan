    ...
    try:
        ...
        if person_name:
            ...
            if len(person_data) == 0:
                return None, f"Person '{person_name}' not found for company '{company_name}'"
...
def validate_report(pdf_path, csv_path, company_name, person_name=None, tolerance=0.15):
    ...
    for key in ['up_avg', 'in_avg', 'do_avg', 'overall_scres']:
        ...
        if exp_val is not None and act_val is not None:
            ...
            if not matches:
                all_match = False
                mismatches.append(f"{label}: Expected={exp_val:.2f}, Actual={act_val:.2f}, Diff={diff:.2f}")
    ...
    if all_match:
        ...
        message = f"[WARNING] {len(mismatches)} mismatch(es) found: " + "; ".join(mismatches)
...
def main():
    ...
    if len(sys.argv) < 4:
        print("Usage: python validate_single_report.py <pdf_path> <csv_path> <company_name> [person_name]")
        sys.exit(1)
    ...
    if result['details']:
        ...
        for key, info in result['details'].items():
            if info['matches']:
                print(f"  [{info['label']:20s}] Expected={info['expected']:.2f}, Actual={info['actual']:.2f} [OK]")
            else:
                if 'note' in info:
                    print(f"  [{info['label']:20s}] {info['note']}")
                else:
                    exp = f"{info['expected']:.2f}" if info['expected'] is not None else "N/A"
                    act = f"{info['actual']:.2f}" if info['actual'] is not None else "N/A"
                    diff = f"Diff={info['diff']:.2f}" if 'diff' in info else ""
                    print(f"  [{info['label']:20s}] Expected={exp}, Actual={act} {diff} [FAIL]")
