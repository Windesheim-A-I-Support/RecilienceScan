...
# --- Draw 5 radars and a text box on one page (2x3 grid) ---
op <- par(no.readonly = TRUE)
on.exit(par(op), add = TRUE)

# Adjust margins for better spacing and to fill the A4 page
par(mfrow = c(2, 3),
    mar = c(1.5, 2, 2.5, 2),  # Increased margins for more space between plots
    oma = c(0.5, 0.5, 0, 0.5), # Added outer margins for a buffer
    cex = 0.9)
...
# 6) Place the text block in the last panel of the grid
# Use a simple plot.new() to move to the next panel
plot.new()
# The coordinates for text() are now relative to this new panel
# They range from 0 (left/bottom) to 1 (right/top)
text_content <- paste(
  "Summary of Resilience Scores\n\n",
  "Overall Score: ", safe_print_score(df_company_numeric$overall_scres), "\n",
  "Upstream Pillar: ", safe_print_score(df_company_numeric$up_pillar_score), "\n",
  "Internal Pillar: ", safe_print_score(df_company_numeric$in_pillar_score), "\n",
  "Downstream Pillar: ", safe_print_score(df_company_numeric$do_pillar_score), "\n\n",
  "Dimensions are rated on a scale from 0 (poor) to 5 (excellent).\n",
  "An average score of 3 or higher indicates a good level of resilience."
)

text(x = 0.5, y = 0.5, labels = text_content, adj = c(0.5, 0.5), cex = 1.0, font=2)
```
