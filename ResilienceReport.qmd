...
for (col in score_cols) {
  if (col %in% colnames(dashboard_data)) {
    ...
    # Replace NA values with neutral score of 2.5 (midpoint)
...
\begin{flushright}
\includegraphics[height=1.5cm]{img/logo-resiliencescan.png}
\end{flushright}
...
{\small\textit{A collaboration between Hogeschool Windesheim, University of Groningen, and Involvation}}

\end{minipage}\hfill
\begin{minipage}[t]{0.26\textwidth}
\vspace{0pt}
\raggedleft
\includegraphics[width=0.75\textwidth]{img/logo-windesheim.png}\\[3mm]
\includegraphics[width=0.30\textwidth]{img/logo-RUG.png}\\[3mm]

\includegraphics[width=0.75\textwidth]{img/logo-involvation.png}
\end{minipage}
...
`r if (detected_language == "dutch") paste("**Sterkste domein:**", if(strongest == "Internal") "Intern" else strongest, paste0("(", sprintf("%.2f", max(pillars)), ") — Behoud deze sterkte")) else paste("**Strongest domain:**", strongest, paste0("(", sprintf("%.2f", max(pillars)), ") — Maintain this strength"))`

`r if (detected_language == "dutch") paste("**Ontwikkelpunt:**", if(weakest == "Internal") "Intern" else weakest, paste0("(", sprintf("%.2f", min(pillars)), ") — Prioriteit voor verbetering")) else paste("**Development priority:**", weakest, paste0("(", sprintf("%.2f", min(pillars)), ") — Priority for improvement"))`

`r gap <- max(pillars) - min(pillars); if (detected_language == "dutch") paste0("**Performance gap:** ", sprintf("%.2f", gap), if(gap > 1.0) " — Aanzienlijke ontwikkelkansen" else " — Redelijk gebalanceerd") else paste0("**Performance gap:** ", sprintf("%.2f", gap), if(gap > 1.0) " — Significant development opportunities" else " — Reasonably balanced")`
:::
...
# Function to format pillar details
format_pillar_details <- function(prefix, pillar_name_nl, pillar_name_en) {
  ...
  if (detected_language == "dutch") {
    cat(sprintf("**%s** (gem: %.2f) — Sterkste: %s (%.2f), Zwakste: %s (%.2f)\n\n",
                pillar_name, mean(scores),
                names(top_2)[1], top_2[1],
                names(bottom_2)[2], bottom_2[2]))
  } else {
    cat(sprintf("**%s** (avg: %.2f) — Strongest: %s (%.2f), Weakest: %s (%.2f)\n\n",
                pillar_name, mean(scores),
                names(top_2)[1], top_2[1],
                names(bottom_2)[2], bottom_2[2]))
  }
...
# Display each pillar summary
format_pillar_details("up", "Upstream", "Upstream")
format_pillar_details("in", "Intern", "Internal")
format_pillar_details("do", "Downstream", "Downstream")
```
