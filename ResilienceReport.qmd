...
tryCatch({
  ...
  if (!dir.exists("data")) {
    ...
    cat("- Data directory: **NOT FOUND** [X]\n")
  } else {
    cat("- Data directory: Found [OK]\n")
  ...
  if (file.exists(data_file_path)) {
    ...
    if ("readr" %in% loaded_packages && is.null(df_raw)) {
      tryCatch({
        ...
        cat("- Strategy 1 (readr UTF-8): **SUCCESS** [OK]\n")
      }, error = function(e) {
    ...
    if ("readr" %in% loaded_packages && is.null(df_raw)) {
      tryCatch({
        ...
        cat("- Strategy 2 (readr auto): **SUCCESS** [OK]\n")
      }, error = function(e) {
    ...
    if (is.null(df_raw)) {
      tryCatch({
        ...
        cat("- Strategy 3 (base R UTF-8): **SUCCESS** [OK]\n")
      }, error = function(e) {
    ...
    if (is.null(df_raw)) {
      tryCatch({
        ...
        cat("- Strategy 4 (base R minimal): **SUCCESS** [OK]\n")
      }, error = function(e) {
...
if (!is.null(df_raw)) {
  ...
  if (nrow(df_raw) == 0) {
    ...
    cat("- **Data content:** Empty dataset [X]\n")
    ...
    cat("- **Data content:** Very small dataset (", nrow(df_raw), " rows) [!]\n")
  } else {
    cat("- **Data content:** Adequate size [OK]\n")
  }
  
  if (ncol(df_raw) == 0) {
    add_validation_message(field = "data_structure", rule = "min_cols > 0", value = "0",
                          severity = "error", message = "Dataset contains no columns")
    cat("- **Data structure:** No columns [X]\n")
  } else {
    cat("- **Data structure:** ", ncol(df_raw), " columns available [OK]\n")
  }
  ...
  cat("\n**[!] Data loading failed - no data available for analysis**\n")
}
...
if (!is.null(df_raw) && nrow(df_raw) > 0) {
  ...
  tryCatch({
    ...
    cat("- **Column name consistency:** ", 
        ifelse(any(duplicated(clean_colnames)), "Duplicates found [X]", "No duplicates [OK]"), "\n")
    ...
    if (length(problematic_names) > 0) {
      ...
      cat("- **Column name issues:** Special characters found in", length(problematic_names), "columns [!]\n")
    } else {
      cat("- **Column name format:** Clean format [OK]\n")
    }
...
if (!is.null(df_raw) && nrow(df_raw) > 0) {
  ...
  tryCatch({
    ...
    if (nrow(problem_columns) > 0) {
      ...
      cat("\n**Data Quality:** No major issues detected [OK]\n")
    }
...
if (!is.null(df_raw) && nrow(df_raw) > 0) {
  ...
  tryCatch({
    ...
    if (length(company_col_candidates) == 0) {
      ...
      cat("- **Status:** No company name columns detected [X]\n")
      ...
      cat("- **Selected column:** ", company_col, " [OK]\n")
      ...
      if (length(exact_matches) > 0) {
        cat("- **Exact match:** Found ", length(exact_matches), " match(es) [OK]\n")
        ...
        if (length(partial_matches) > 0) {
          cat("- **Partial match:** Found ", length(partial_matches), " match(es) [!]\n")
...
if (!is.null(company_data) && nrow(company_data) > 0) {
  ...
  tryCatch({
    ...
    cat("- **Company found:** ", company_data[[company_col]][1], " [OK]\n")
  ...
  cat("**[!] No company data extracted - see matching issues above**\n\n")
}
```
...
tryCatch({
  ...
  if (length(validation_results$detailed_messages) > 0) {
    ...
    cat("\n**[OK] No validation issues detected**\n")
  }
  ...
  cat("[!] **Error generating validation summary:** ", e$message, "\n")
})
...
# LaTeX escaping function for special characters
...
# Create LaTeX-safe versions of company and person names
...
::: {.content-visible when-format="pdf"}
% NEXT Gen Resilience logo centered at top - compact size
\begin{center}
...
% Company info and partner logos - compact layout
\noindent
...
`r if (nchar(person_name) > 0) { if (nchar(person_function) > 0) paste("\\textbf{This Report:}", person_name_safe, "---", escape_latex(person_function), "\\\\") else paste("\\textbf{This Report:}", person_name_safe, "\\\\") }`
...
\raggedleft
\includegraphics[width=0.75\textwidth]{img/logo-windesheim.png}\\[1.5mm]
\includegraphics[width=0.75\textwidth]{img/logo-involvation.png}\\[1.5mm]
\includegraphics[width=0.75\textwidth]{img/logo-RUG.png}
\end{minipage}
...
tryCatch({
  if ("fmsb" %in% loaded_packages) {
    ...
    create_radar <- function(prefix, title, color) {
      ...
      # fmsb::radarchart plots first column at TOP and goes clockwise
      # To get R-C-F-V-A clockwise from top, use that exact order
      radar_data <- data.frame(
    ...
    # First column plots at top, then clockwise
  ...
  tryCatch({
    if ("fmsb" %in% loaded_packages) {
      ...
      create_company_radar <- function(prefix, title, color) {
        ...
        # First column at top, then clockwise
        radar_data <- data.frame(
          Redundancy = c(5, 0, scores[1]),
          Collaboration = c(5, 0, scores[2]),
          Flexibility = c(5, 0, scores[3]),
