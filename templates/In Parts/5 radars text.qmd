...
# Compact layout for recommendations and financial impact
cat("\\begin{minipage}[t]{0.58\\textwidth}\n")
cat("\\textbf{Strategic Recommendations}\\\\\n")
cat("\\footnotesize\\textbf{Priority Enhancement Areas:}\\\\\n")

for (area in lowest_areas) {
  score <- all_scores[area]
  cat("\\textbullet\\ \\textbf{", area, " (", sprintf("%.1f", score), "):} ")
  
  if (grepl("Redundancy", area)) {
    cat("Backup suppliers, buffer inventory, alternative routes\\\\\n")
  } else if (grepl("Collaboration", area)) {
    cat("Enhanced information sharing, joint planning, communication systems\\\\\n")
  } else if (grepl("Flexibility", area)) {
    cat("Flexible contracts, modular processes, cross-training\\\\\n")
  } else if (grepl("Visibility", area)) {
    cat("Monitoring systems, analytics, end-to-end transparency\\\\\n")
  } else if (grepl("Agility", area)) {
    cat("Streamlined decisions, response protocols, change management\\\\\n")
  }
}

cat("\\end{minipage}\\hfill\n")
cat("\\begin{minipage}[t]{0.38\\textwidth}\n")
cat("\\textbf{Financial Impact}\\\\\n")
cat("\\footnotesize\\textbf{Risk Mitigation Value:}\\\\\n")
cat("\\textbullet\\ Reduced disruption costs\\\\\n")
cat("\\textbullet\\ Lower expediting fees\\\\\n") 
cat("\\textbullet\\ Improved delivery performance\\\\\n")
cat("\\textbullet\\ Enhanced customer retention\\\\\n")
cat("\\textbullet\\ Optimized working capital\\\\\n")
cat("\\textbullet\\ Competitive reliability advantage\\\\\n")
cat("\\end{minipage}\n\n")
