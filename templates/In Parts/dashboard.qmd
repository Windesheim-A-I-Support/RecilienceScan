...
if (file.exists(data_file)) {
  tryCatch({
    ...
    if (length(company_cols) > 0) {
      ...
      exact_matches <- which(tolower(trimws(df_master[[company_col]])) == tolower(trimws(company_target)))

      if (length(exact_matches) > 0) {
        company_data_extracted <- df_master[exact_matches[1], , drop = FALSE]
      } else {
        # Try partial match
        partial_matches <- which(grepl(tolower(trimws(company_target)), tolower(trimws(df_master[[company_col]])), fixed = TRUE))
