...
def main():
    ...
    for item in test_companies:
        ...
        for key, result in comparison.items():
            ...
            if result['matches']:
                print(f"   [OK] {label}: Expected={result['expected']:.2f}, Actual={result['actual']:.2f}")
            else:
                if 'error' in result:
                    print(f"   [FAIL] {label}: {result['error']}")
                else:
                    print(f"   [FAIL] {label}: Expected={result['expected']:.2f}, Actual={result['actual']:.2f}, Diff={result['diff']:.2f}")
                all_match = False
                all_passed = False
