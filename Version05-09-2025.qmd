...
```{r dashboard-radars, fig.width=16, fig.height=20}
# Dashboard layout with all radar charts - optimized for A4
op <- par(no.readonly = TRUE)
on.exit(par(op), add = TRUE)

# Set up 3x2 grid layout (3 rows, 2 columns) with generous spacing
par(mfrow = c(3, 2), oma = c(3, 3, 5, 3))

# Chart 1: Overall (larger, top-left)
...
# Chart 2: Comparative Overlay (top-right)
if (!is.null(overlay_data)) {
  ...
  legend("bottomright",
    ...
    col = cols[1:n_series],
    lty = 1, lwd = 3, bty = "n", cex = 1.0, text.col = "grey20")
} else {
  plot.new()
  title("Comparative Analysis\n(no data available)", cex.main = 1.2)
}

# Chart 3: Upstream (middle-left)
if (has_up) {
  par(mar = c(3, 3, 4, 3))
  fmsb::radarchart(
    ...
    cglcol = "grey80", cglty = 1, axislabcol = "grey40",
    caxislabels = sprintf("%.1f", seq(min_score, max_score, length.out = 6)),
    cglwd = 1.2, vlcex = 1.2, centerzero = TRUE, title = "Upstream Supply Chain"
  )
} else {
  plot.new()
...
if (has_in) {
  ...
  fmsb::radarchart(
    ...
    cglcol = "grey80", cglty = 1, axislabcol = "grey40",
    caxislabels = sprintf("%.1f", seq(min_score, max_score, length.out = 6)),
    cglwd = 1.2, vlcex = 1.2, centerzero = TRUE, title = "Internal Operations"
  )
} else {
  plot.new()
...
# Chart 5: Downstream (bottom-left)
if (has_do) {
  par(mar = c(3, 3, 4, 3))
  fmsb::radarchart(
    ...
    cglcol = "grey80", cglty = 1, axislabcol = "grey40",
    caxislabels = sprintf("%.1f", seq(min_score, max_score, length.out = 6)),
    cglwd = 1.2, vlcex = 1.2, centerzero = TRUE, title = "Downstream Distribution"
  )
} else {
  plot.new()
...
# Chart 6: Summary Statistics (bottom-right)
par(mar = c(3, 3, 4, 3))
plot.new()

# Create a more readable summary with better formatting
rect(0.05, 0.05, 0.95, 0.95, col = "#f8f9fa", border = "grey70", lwd = 2)

# Company header
text(0.5, 0.85, params$company, cex = 1.6, font = 2, col = "#2c3e50")
text(0.5, 0.78, paste("Sector:", company_sector), cex = 1.1, col = "grey40")
text(0.5, 0.72, paste("Size:", company_size), cex = 1.1, col = "grey40")

# Horizontal line
segments(0.15, 0.67, 0.85, 0.67, col = "grey60", lwd = 2)

# Pillar scores with color coding
y_positions <- c(0.58, 0.50, 0.42, 0.34, 0.26)
labels <- c("OVERALL RESILIENCE", "Upstream Pillar", "Internal Pillar", "Downstream Pillar", "")
scores <- c(
...
colors <- c("#6A1B9A", "#0277BD", "#FF8F00", "#2E7D32", "black")

for (i in 1:4) {
  # Score label
  text(0.25, y_positions[i], labels[i], cex = 1.2, font = ifelse(i==1, 2, 1), 
  ...
  if (score_val != "N/A" && !is.na(as.numeric(score_val))) {
    ...
    text(0.75, y_positions[i], score_val, cex = 1.4, font = 2, col = "white")
  } else {
    text(0.75, y_positions[i], score_val, cex = 1.2, font = 1, col = "grey50")
  }
}

# Scale information
text(0.5, 0.18, "PERFORMANCE SCALE", cex = 1.0, font = 2, col = "grey30")
text(0.5, 0.12, "0.0 - 2.9: Needs Improvement", cex = 0.9, col = "#e74c3c")
text(0.5, 0.08, "3.0 - 3.9: Good Performance", cex = 0.9, col = "#f39c12")
text(0.5, 0.04, "4.0 - 5.0: Excellence", cex = 0.9, col = "#27ae60")

# Add main title
mtext(paste("Supply Chain Resilience Dashboard"), 
      outer = TRUE, cex = 2.0, font = 2, line = 2)
