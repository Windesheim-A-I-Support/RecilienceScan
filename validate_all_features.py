...
class FeatureValidator:
    ...
    def test_quality_dashboard_script(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if png_files:
                    self.log("PASS", "Quality dashboard script runs",
                            f"Created {len(png_files)} dashboard file(s)")
                    return True
                else:
                    self.log("FAIL", "Quality dashboard script runs",
    ...
    def test_data_cleaner_script(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if log_file.exists():
                    self.log("PASS", "Data cleaner script runs",
                ...
                self.log("FAIL", "Data cleaner script runs",
                        f"Exit code: {result.returncode}\n{result.stderr}")
                test_file.unlink(missing_ok=True)
                return False
        ...
        try:
            ...
            if 'debug_mode:' in content:
                ...
                if 'params$debug_mode' in content and 'debug-table' in content:
                    self.log("PASS", "Debug mode parameter configured",
                            "Found debug_mode param and conditional table")
                    return True
                else:
                    self.log("FAIL", "Debug mode parameter configured",
        ...
        try:
            ...
            if 'diagnostic_mode:' in content:
                ...
                if 'params$diagnostic_mode' in content:
                    self.log("PASS", "Demo mode parameter configured",
                            "Found diagnostic_mode param and usage")
                    return True
                else:
                    self.log("FAIL", "Demo mode parameter configured",
        ...
        try:
            ...
            if 'person:' in content:
                ...
                if 'person_target' in content and 'normalize_name' in content:
                    self.log("PASS", "Person parameter configured",
                            "Found person param and filtering logic")
                    return True
                else:
                    self.log("FAIL", "Person parameter configured",
    ...
    def test_robust_data_cleaning(self):
        ...
        try:
            ...
            if has_gsub_comma and has_gsub_nonnumeric and has_na_replacement and has_clamping:
                self.log("PASS", "Robust data cleaning implemented",
                        "Found all cleaning steps: comma replacement, non-numeric removal, NA handling, clamping")
                return True
            else:
                missing = []
                if not has_gsub_comma: missing.append("comma replacement")
                if not has_gsub_nonnumeric: missing.append("non-numeric removal")
                if not has_na_replacement: missing.append("NA handling")
                if not has_clamping: missing.append("value clamping")

                self.log("FAIL", "Robust data cleaning implemented",
    ...
    def test_gui_checkboxes(self):
        ...
        try:
            ...
            if has_debug_var and has_demo_var and has_debug_checkbox and has_demo_checkbox:
                self.log("PASS", "GUI checkboxes configured",
                        "Found debug_mode_var, demo_mode_var and both checkboxes")
                return True
            else:
                missing = []
                if not has_debug_var: missing.append("debug_mode_var")
                if not has_demo_var: missing.append("demo_mode_var")
                if not has_debug_checkbox: missing.append("debug checkbox")
                if not has_demo_checkbox: missing.append("demo checkbox")

                self.log("FAIL", "GUI checkboxes configured",
                        f"Missing: {', '.join(missing)}")
                return False
    ...
    def test_gui_quality_buttons(self):
        ...
        try:
            ...
            if has_quality_button and has_cleaner_button and has_quality_method and has_cleaner_method:
                self.log("PASS", "GUI quality buttons configured",
                        "Found both buttons and their handler methods")
                return True
            else:
                missing = []
                if not has_quality_button: missing.append("quality button")
                if not has_cleaner_button: missing.append("cleaner button")
                if not has_quality_method: missing.append("quality method")
                if not has_cleaner_method: missing.append("cleaner method")

                self.log("FAIL", "GUI quality buttons configured",
                        f"Missing: {', '.join(missing)}")
                return False
    ...
    def test_gui_passes_parameters(self):
        ...
        try:
            ...
            if has_person_param and has_debug_param and has_demo_param:
                self.log("PASS", "GUI passes all parameters to Quarto",
                        "Found person, debug_mode, and diagnostic_mode parameters")
                return True
            else:
                missing = []
                if not has_person_param: missing.append("person parameter")
                if not has_debug_param: missing.append("debug_mode parameter")
                if not has_demo_param: missing.append("diagnostic_mode parameter")

                self.log("FAIL", "GUI passes all parameters to Quarto",
                        f"Missing: {', '.join(missing)}")
                return False
    ...
    def test_generate_all_reports_passes_person(self):
        ...
        try:
            ...
            if '-P person=' in content or "'-P', f'person=" in content or 'f"-P person=' in content:
                self.log("PASS", "Generate_all_reports passes person parameter",
                        "Found person parameter in Quarto command")
                return True
            else:
                self.log("FAIL", "Generate_all_reports passes person parameter",
    ...
    def test_email_priority_fallback(self):
        ...
        try:
            ...
            if has_priority_list and has_info_email and has_rdeboer and has_cgverhoef and has_fallback_logic:
                self.log("PASS", "Email priority fallback configured",
                        "Found priority accounts and fallback logic")
                return True
            else:
                missing = []
                if not has_priority_list: missing.append("priority_accounts list")
                if not has_info_email: missing.append("info@resiliencescan.org")
                if not has_rdeboer: missing.append("r.deboer@windesheim.nl")
                if not has_cgverhoef: missing.append("cg.verhoef@windesheim.nl")
                if not has_fallback_logic: missing.append("fallback logic")

                self.log("FAIL", "Email priority fallback configured",
                        f"Missing: {', '.join(missing)}")
                return False
    ...
    def generate_report(self):
        ...
        with open(report_file, 'w', encoding='utf-8', errors='replace') as f:
            ...
            f.write("="*70 + "\n")
            f.write(f"Total: {self.test_count} | Passed: {self.pass_count} | Failed: {self.fail_count}\n")
